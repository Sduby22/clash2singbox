...
impl<'de> Visitor<'de> for RuleVisitor {
    ...
    where
        ...
        if strs.len() < 3 {
            Ok(Rule{category: strs[0].to_owned(), content: "".to_owned(), target: strs[1].to_owned(), no_resolve: false})
        }
        else {
            Ok(Rule{category: strs[0].to_owned(), content: strs[1].to_owned(), target: strs[2].to_owned(), no_resolve: if strs.len() > 3 {true} else {false}})
        }
    }
}
